...
LRESULT CALLBACK WndProcNOT(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	LRESULT result = 0;
	switch (msg)
	{
...
int WINAPI WinMainNOT(HINSTANCE hInstance, HINSTANCE /*hPrevInstance*/, LPSTR /*lpCmdLine*/, int /*nShowCmd*/)
{
	// Open a window
	HWND hwnd;
	{
		...
		winClass.lpfnWndProc = &WndProcNOT;
	...
	ID3D11VertexShader* vertexShader;
	{
		...
		HRESULT hResult = D3DCompileFromFile(L"Source/Private/Shader/Shader.hlsl", nullptr, nullptr, "vs_main", "vs_5_0", 0, 0, &vsBlob, &shaderCompileErrorsBlob);
		if (FAILED(hResult))
		{
	...
	ID3D11PixelShader* pixelShader;
	{
		ID3DBlob* psBlob;
		ID3DBlob* shaderCompileErrorsBlob;
		HRESULT hResult = D3DCompileFromFile(L"Source/Private/Shader/Shader.hlsl", nullptr, nullptr, "ps_main", "ps_5_0", 0, 0, &psBlob, &shaderCompileErrorsBlob);
		if (FAILED(hResult))
		{
			const char* errorString = NULL;
			if (hResult == HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND))
